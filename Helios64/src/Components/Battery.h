...
namespace Helios
{
    ...
    const unsigned short int battery_level_glyphs_horizontal[] = {
    ...
    // Segoe MDL2 icon list https://docs.microsoft.com/en-us/windows/uwp/design/style/segoe-ui-symbol-font
    const unsigned short int battery_level_glyphs_vertical[] = {
    ...
    public:
        ...
        {
            Helios::Resources::RenderGlyph(this->monitor, GetBatteryGlyph(battery_percentage), this->transform.x, this->transform.y + 7);
            std::string percent_text = std::to_string(battery_percentage) + "%";
            Helios::Resources::RenderText(this->monitor, percent_text.c_str(), this->transform.x + 26, this->transform.y + 4);
    ...
    private:
        ...
        const unsigned short int GetBatteryGlyph(double percent) {
            const unsigned char percent_level = PercentageToLevel(percent);
            const unsigned short int* glyph_set = (battery_orientation) ? battery_level_glyphs_vertical : battery_level_glyphs_horizontal;
